...
// An opaque data structure to pass to read/write functions
typedef struct pcisel k8_pcidev_t;
...
int k8_pci_vendor_device_list(int vendor_id, int device_id, k8_pcidev_t devs[])
{
	int matches = -1;
...
k8_pci_read(k8_pcidev_t dev, int offset, int *data, int width)
{
	struct pci_io ctrl;
...
// same, but for writes.
k8_pci_write(k8_pcidev_t dev, int offset, int data, int width)
{
	struct pci_io ctrl;
	bzero(&ctrl, sizeof(ctrl));
